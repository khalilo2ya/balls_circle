...
def create_ball(size):
    # Middle of the top edge of the circle
    start_x = circle_center[0]
    start_y = circle_center[1] - circle_radius + size  # Slightly away from the edge
    
    vx, vy = generate_velocity(speed)
    return Ball(start_x, start_y, vx, vy,
                (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255)), size)

# Example of ball sizes
initial_ball_size = 10
balls = [create_ball(initial_ball_size)]
...
while running:
    ...
    for ball in balls:
        if ball.update():
            # Ball escaped through the gap, create exactly two new balls with consistent speed and random size
            new_balls.append(create_ball(random.randint(5, 15)))  # Random size between 5 and 15
            new_balls.append(create_ball(random.randint(5, 15)))  # Random size between 5 and 15
